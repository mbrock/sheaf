...
export const ParagraphEditor = {
  mounted() {
    ...
    const initialDoc =
      this.format === "markup"
        ? docFromMarkup(this.initialMarkup)
        : docFromText(this.initialText)
    this.initialSerializedMarkup =
      this.format === "markup" ? markupFromDoc(initialDoc) : ""
    this.view = new EditorView(
      this.el.querySelector("[data-paragraph-editor-surface]"),
      {
  ...
  updateSaveState() {
    ...
    const current =
      this.format === "markup"
        ? markupFromDoc(this.view.state.doc)
        : textFromDoc(this.view.state.doc)
    const initial =
      this.format === "markup"
        ? this.initialSerializedMarkup
        : this.initialText
...
function docFromText(text) {
  const blocks = `${text}`.split(/\n{2,}/).map((paragraph) => {
    ...
    return editorSchema.node(
      "paragraph",
      null,
      normalized ? [editorSchema.text(normalized)] : [],
    )
  })
  ...
  return editorSchema.node(
    "doc",
    null,
    blocks.length ? blocks : [editorSchema.node("paragraph")],
  )
}
