defmodule Sheaf.ThesisTest do
  ...
  test "from_rows reconstructs nested blocks in rdf:Seq order" do
    ...
    rows = [
      ...
      triple(first_paragraph, RDF.type(), resolve(SheafNS.Paragraph)),
      triple(first_paragraph, SheafNS.text(), RDF.literal("Opening paragraph.")),
      ...
      triple(nested_paragraph, RDF.type(), resolve(SheafNS.Paragraph)),
      triple(nested_paragraph, SheafNS.text(), RDF.literal("Nested paragraph.")),
      triple(tail_paragraph, RDF.type(), resolve(SheafNS.Paragraph)),
      triple(tail_paragraph, SheafNS.text(), RDF.literal("Trailing paragraph."))
    ]
    ...
    assert [%{type: :section, heading: "Introduction"}, %{type: :paragraph, text: "Trailing paragraph."}] =
             document.children

    [intro_block, trailing_block] = document.children

    assert [%{type: :paragraph, text: "Opening paragraph."}, %{type: :section, heading: "Research Questions"}] =
             intro_block.children

    assert trailing_block.text == "Trailing paragraph."
    assert [%{type: :paragraph, text: "Nested paragraph."}] = Enum.at(intro_block.children, 1).children
  end
