defmodule Sheaf.ThesisTest do
  ...
  test "navigates nested document blocks in RDF list order" do
    ...
    assert Thesis.title(graph, thesis) == "Example Thesis"
    assert Thesis.kind(graph, thesis) == :thesis

    assert [^intro, ^tail_paragraph] = Thesis.children(graph, thesis)
    assert Thesis.block_type(graph, intro) == :section
    assert Thesis.heading(graph, intro) == "Introduction"
    assert Thesis.block_type(graph, tail_paragraph) == :paragraph
    assert Thesis.paragraph_text(graph, tail_paragraph) == "Trailing paragraph."

    assert [^first_paragraph, ^nested_section] = Thesis.children(graph, intro)
    assert Thesis.block_type(graph, first_paragraph) == :paragraph
    assert Thesis.paragraph_text(graph, first_paragraph) == "Opening paragraph."
    assert Thesis.block_type(graph, nested_section) == :section
    assert Thesis.heading(graph, nested_section) == "Research Questions"
