defmodule Sheaf.Assistant.ChatTest do
  ...
  alias ReqLLM.{Context, Response, StreamChunk, StreamResponse, Tool, ToolResult}
  ...
  test "finished present spreadsheet tool messages keep the renderable result" do
    ...
    generate_text = fn _model, _context, _opts ->
      GenServer.cast(
        Server.via(id),
        {:assistant_event,
         {:tool_started, "present_spreadsheet_query_result", %{"title" => "Agency rows"}}}
      )
    ...
    start_supervised!(
      {Chat,
       ...
       model: "test-model",
       titles: %{},
       workspace_instructions: "Testing presented spreadsheet result visibility.",
       activity_writer: nil,
       generate_text: generate_text,
       task_supervisor: Sheaf.Assistant.TaskSupervisor}
    )
    ...
    assert %{
           ...
           } = Enum.find(messages, &(Map.get(&1, :tool) == "present_spreadsheet_query_result"))
  end
