  ...
  test "builds a spreadsheet document grouped by source sections" do
    ...
    result =
      Sheaf.Spreadsheet.build_graph(rows,
        ...
        mint: fn -> Agent.get_and_update(minted, fn [iri | rest] -> {iri, rest} end) end
      )
    ...
    assert rdf_value(Graph.description(graph, document), RDFS.label()) == "IEVA coded excerpts"
    assert Description.include?(Graph.description(graph, document), {RDF.type(), DOC.Spreadsheet})
    assert [^agnese_section, ^liga_section] = Sheaf.Document.children(graph, document)

    assert rdf_value(Graph.description(graph, agnese_section), RDFS.label()) == "Agnese Z."

    assert [^first_row, ^second_row] = Sheaf.Document.children(graph, agnese_section)

    first_row_description = Graph.description(graph, first_row)

    assert Description.include?(first_row_description, {RDF.type(), DOC.Row})
    assert rdf_value(first_row_description, DOC.text()) == "First marked text."
    assert rdf_value(first_row_description, DOC.spreadsheetRow()) == 2
    assert rdf_value(first_row_description, DOC.spreadsheetSource()) == "Agnese Z."
    assert rdf_value(first_row_description, DOC.codeCategory()) == "RQ1-1"
    assert rdf_value(first_row_description, DOC.sourceKey()) == "priv/ieva_data/coded.csv#row=2"
  end
