...
const cli = new URL("../bin/sheaf.ts", import.meta.url).pathname
...
describe("sheaf CLI", () => {
  test("maps documents to the research-library tool and prints text", async () => {
    const requests: RequestRecord[] = []
    const server = mockServer(requests, {
      content: [
        { type: "text", text: "# Research library\n\nABC123 Example" },
      ],
      isError: false,
    })

    const result = await runCli(server, ["documents"])
  ...
  test("reports authentication failures without exposing the token", async () => {
    ...
    const result = await runCli(server, ["notes"])
...
async function runCli(
  ...
  const process = Bun.spawn(["bun", cli, ...args], {
    env: {
      ...Bun.env,
      SHEAF_URL: `http://127.0.0.1:${server.port}`,
      SHEAF_TOKEN: "test-token",
      SHEAF_HOST: "",
      SHEAF_MCP_TOKEN: "",
    },
