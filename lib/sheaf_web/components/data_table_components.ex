defmodule SheafWeb.DataTableComponents do
      ...
      <figure class="flex w-full max-w-full flex-col items-center">
        ...
        >
          <div class="min-w-0 flex-1">
            <p :if={present?(@title)} class="font-medium text-stone-950 dark:text-stone-50">
  ...
  defp caption_text(assigns) do
    ...
    ~H"""
    <span :for={part <- @parts}>
      <code :if={part.kind == :identifier} class="font-mono text-stone-800 dark:text-stone-100">
        {part.text}
      </code>
  ...
  defp caption?(assigns) do
    present?(assigns[:title]) or present?(assigns[:subtitle]) or present?(assigns[:href])
  end
